"""
Implement Client for Subtraction MicroService.

Implement MicroserviceClientSubtraction class and the ROS 2 command:
    microservices_py client_Subtraction
"""
import random
...
class MicroServiceClientSubtraction(Node):
    """
    Client of Subtraction microservice.

    This Node creates a clients that connects to an SubtractionMicroService.
    ...
    def __init__(
        ...
        This create the service 'Subtraction_service' and
        register the callback function 'Subtraction_service_callback'
        to process with every request.
        """
        # Call Node constructor
        super().__init__('SubtractionMicroClient')

        # Create both clients
        self.subtraction_client = self.create_client(
            AddTwoInts, 'subtraction_service')
    ...
    def wait_to_find_server(
        ...
        """
        while not self.subtraction_client.wait_for_service(timeout_sec=1.0):
    ...
    def send_request(
        ...
        # Send request
        self.future = self.subtraction_client.call_async(request)
...
def main():
    """
    Create and execute SubtractionService.
    ...
    # Create Client Node
    minimal_client = MicroServiceClientSubtraction()
