.. _notes_kilted_latest:

Kilted Kolumbo (v5.1.0)
=======================

This version ships the following packages:

.. list-table::
    :header-rows: 1

    * - Package name
      - Package version
      - Since Vulcanexus version
    * - eProsima Fast CDR
      - `v2.3.4 <https://github.com/eProsima/Fast-CDR/releases/tag/v2.3.4>`__
      - v5.1.0
    * - eProsima Fast DDS
      - `v3.4.1 <https://fast-dds.docs.eprosima.com/en/3.x/notes/previous_versions/supported_versions.html>`__
      - v5.1.0
    * - eProsima Fast DDS Python
      - `v2.4.1 <https://github.com/eProsima/Fast-DDS-python/releases/tag/v2.4.1>`__
      - v5.1.0
    * - eProsima Fast DDS Gen
      - `v4.2.0 <https://github.com/eProsima/Fast-DDS-Gen/releases/tag/v4.2.0>`__
      - v5.1.0
    * - foonathan_memory_vendor
      - `v1.3.1 <https://github.com/eProsima/foonathan_memory_vendor/releases/tag/v1.3.1>`__
      - v4.0.0
    * - micro-ROS Agent
      - `6.1.0 <https://github.com/micro-ROS/micro-ROS-Agent/blob/6.1.0/micro_ros_agent/CHANGELOG.rst#610-2025-07-10>`__
      - v4.0.0
    * - micro-ROS Setup
      - `6.0.1 <https://github.com/micro-ROS/micro_ros_setup/blob/6.0.1/CHANGELOG.rst#601-2025-12-26>`__
      - v5.1.0
    * - Micro-XRCE-DDS-Agent
      - `3.0.1 <https://micro-xrce-dds.docs.eprosima.com/en/latest/notes.html#version-3-0-1>`__
      - v5.0.0
    * - Micro-XRCE-DDS-Gen
      - `2.0.2 <https://github.com/eProsima/Micro-XRCE-DDS-Gen/releases/tag/v2.0.2>`__
      - v5.0.0
    * - Micro-ROS-Msgs
      - `6.0.0 <https://github.com/micro-ROS/micro_ros_msgs/blob/kilted/CHANGELOG.rst#600-2024-05-31>`__
      - v4.0.0
    * - ROS 2 Monitor
      - `v3.4.0 <https://fast-dds-monitor.readthedocs.io/en/latest/rst/notes/notes.html#version-v3-4-0>`__
      - v5.1.0
    * - ROS 2 Router
      - `v3.4.0 <https://eprosima-dds-router.readthedocs.io/en/latest/rst/notes/notes.html#version-v3-4-0>`__
      - v5.1.0
    * - ROS 2 Shapes Demo
      - `v3.4.1 <https://eprosima-shapes-demo.readthedocs.io/en/latest/notes/previous_versions/supported_versions.html#version-3-4>`__
      - v5.1.0
    * - ROS 2 Shapes Demo TypeSupport
      - `v1.0.0 <https://github.com/eProsima/ShapesDemo-TypeSupport/releases/tag/v1.0.0>`__
      - v5.0.0
    * - ROS 2 Statistics Backend
      - `v2.4.0 <https://fast-dds-statistics-backend.readthedocs.io/en/latest/rst/notes/notes.html>`__
      - v5.1.0
    * - Vulcanexus Fast DDS RMW
      - `5.1.0 <https://github.com/eProsima/rmw_fastrtps/releases/tag/v5.1.0>`__
      - v5.1.0
    * - eProsima Dev Utils
      - `v1.4.0 <https://github.com/eProsima/dev-utils/releases/tag/v1.4.0>`__
      - v5.1.0
    * - eProsima DDS Pipe
      - `v1.4.0 <https://github.com/eProsima/DDS-Pipe/releases/tag/v1.4.0>`__
      - v5.1.0
    * - eProsima DDS Record & Replay
      - `v1.4.0 <https://dds-recorder.readthedocs.io/en/latest/rst/notes/notes.html#version-v1-4-0>`__
      - v5.1.0
    * - eProsima Fast DDS Spy
      - `v1.4.0 <https://fast-dds-spy.readthedocs.io/en/latest/rst/notes/notes.html#version-v1-4-0>`__
      - v5.1.0
    * - eProsima Fast DDS Visualizer Plugin
      - `v2.3.0 <https://github.com/eProsima/fastdds-visualizer-plugin/releases/tag/v2.3.0>`__
      - v5.1.0
    * - PlotJuggler
      - `3.9.1 <https://github.com/facontidavide/PlotJuggler/releases/tag/3.9.1>`__
      - v5.0.0
    * - Vulcanexus IDL Dynamic TypeSupport Fast DDS
      - `0.4.2 <https://github.com/eProsima/rosidl_dynamic_typesupport_fastrtps/releases/tag/0.4.2>`__
      - v5.1.0
    * - Vulcanexus Base
      - `v5.1.0 <https://docs.vulcanexus.org/en/latest/rst/notes/kilted/notes.html#kilted-kolumbo-v5-1-0>`__
      - v5.1.0

Kilted Kolumbo previous versions
================================

.. include:: previous_versions/v5.0.0.rst
