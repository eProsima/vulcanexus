...
The cloud router is launched from within a *Vulcanexus Cloud* Docker image (that can be downloaded in `Vulcanexus webpage <https://www.vulcanexus.com>`_), which uses as configuration file the one hosted in the previously set up ConfigMap.
