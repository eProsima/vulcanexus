...
#.  Setting up the development environment on the local host. For this second option, it is necessary to have installed the ``vucanexus-kilted-desktop`` package, since this is the one that includes all the simulation tools, demos and tutorials.
...
The application is based on the `ROS 2 tutorials <https://github.com/ros/ros_tutorials/tree/kilted/turtlesim>`_, but has been slightly modified in order to make it easier to understand and adapt to the ROS 2 and modern C++ programming methods.
...
.. code-block:: bash
    ...
    git clone --branch kilted https://github.com/eProsima/vulcanexus.git
