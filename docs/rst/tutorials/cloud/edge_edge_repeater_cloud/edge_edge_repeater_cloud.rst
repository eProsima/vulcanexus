...
It will focus on an edge-edge architecture in which both edge robots deployed on different networks (WAN) with access to the Internet,
and a server in the Cloud reachable through the Internet.

The scenario we want to simulate in this tutorial is the one where the edges are not connected directly
(are under different LANs) and they do not have access to the locations and networks of each other, due to
intermediate NATs or dynamic networks.
In this scenario, the Cloud server will work as a **TURN** (Traversal Using Relays around NAT), a.k.a **Repeater**.
This kind of servers are meant to be accessed from every point in the WAN network, and will forward the messages from
any edge to the rest of them connected.
Thus, it creates a bridge between 2 networks that do not have direct access to each other, facilitating network configuration,
as NATs Traversals and dynamic addresses.
...
.. warning::
    ...
    However, if there is only access to a LAN communication, it is possible to follow the tutorial by changing the ROS 2 Domain Ids so that the edges uses the different Domains each (``0`` and ``1``).
    This way the ROS 2 nodes are logically isolated and will not discovery other nodes out of their ROS 2 Domain.

This way, all the elements involved in this architecture will be studied, starting with the edge robots.
...
Continuing with the intermediate elements that enable communication over the Internet, in one of each edge and in Cloud.
...
1.  **ROS 2 Router / DDS Router**.
    *eProsima ROS 2 Router*, a.k.a `DDS Router <https://github.com/eProsima/DDS-Router>`_, is an end-user software application that enables the connection of distributed ROS 2 networks (see DDS Router documentation `here <https://eprosima-dds-router.readthedocs.io/en/latest/>`).
    ...
    * *DDS Router Repeater*. It plays the TURN server role in the communication. It will expose some public network addresses to which the edge Routers could establish communication, and will forward the message from one LAN to the other.
...
The ``participants`` are the interfaces of the DDS Router to communicate with other networks. In this case, we have two participants:
    ...
    *   ``local``: this is a simple participant that communicates with all ROS 2 nodes it finds in domain 1.
...
.. todo::
    ...
    decide wether to link to previous tutorial or reexplain it here
...
As the Repeater server is the same for both edges, the configuration of the DDS Router Edge 2 is similar same as
...
In order to communicate both edges, we use a DDS Router configuring as a *Repeater* that forward the messages from one edge to the other.
This machine should be accessible from the Internet.
In case this device is working under a NAT, check :ref:`previous turotial <tutorials_cloud_wan_edge_cloud_wan_edge_cloud_configure_transversal_nat>`
for more information about how to configure the NAT to be accessible from the outside.
The following snippet shows a configuration file for this DDS Router:
...
Once the discovery has been achieved, this Repeater will forward the data from one edge to the other.

To run the DDS Router with the configuration file created as an argument, execute the following command after sourcing the Vulcanexus environment:
...
.. note::
    ...
    resending redundant information or sending back any message.
