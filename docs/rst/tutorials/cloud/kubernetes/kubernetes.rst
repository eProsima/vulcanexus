...
Ensure that the Vulcanexus installation includes the cloud and the ROS 2 demo nodes package (it is suggested to use ``vulcanexus-kilted-desktop``).
...
Assuming the name of the generated Docker image is ``ubuntu-vulcanexus-cloud:kilted``, the cloud router will then be deployed with the following settings:
...
.. code-block:: bash
    ...
    docker build -t vulcanexus-cloud-demo-nodes:kilted -f Dockerfile
